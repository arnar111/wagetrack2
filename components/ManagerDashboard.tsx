...
import { 
  ...
  Trophy, Activity, Heart, Sword, ShoppingBag, PieChart as PieIcon, Settings, Save, X, DollarSign, ArrowRightCircle, ChevronDown, ChevronUp, Eye, EyeOff, BrainCircuit
} from 'lucide-react';
...
const ManagerDashboard: React.FC<ManagerDashboardProps> = ({ allShifts, allSales, allUsers, currentUser }) => {
  ...
  const [teamGoals, setTeamGoals] = useState<TeamGoals>({ monthly: 5000000 });
  ...
  const displayData = useMemo(() => {
    ...
    users.forEach(u => {
      ...
      const proj = Math.random() > 0.4 ? 'Hringurinn' : 'Verið';
      sales.push({ id: `sa-${u.id}`, userId: u.staffId, date: today, timestamp: new Date().toISOString(), amount, project: proj });
    });
  ...
  useEffect(() => {
    const runAI = async () => {
      ...
      setIsLoadingAI(true);
      const res = await getManagerCommandAnalysis({ projectStats, totalTeamSales, topAgent: agentLeaderboard[0]?.name });
      setAiPulse(res);
      setIsLoadingAI(false);
    };
    runAI();
  }, [isDemoMode, displayData.sales.length, activeTab]);
    ...
    <div className="space-y-8 pb-32">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <p className="text-[10px] font-black text-[#d4af37]/60 uppercase tracking-[0.4em] mt-2">Yfirsýn og Verkefnastýring</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex bg-[#0f172a] p-1.5 rounded-[24px] border border-white/5">
            {[
              ...
              { id: 'projects', label: 'Verkefni', icon: <Sword size={16} /> },
        ...
        {activeTab === 'overview' && (
          <motion.div key="overview" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <Speedometer value={totalTeamSales} max={teamGoals.monthly} label="Mánaðarmarkmið í höfn" />
              ...
              <div className="glass p-10 rounded-[48px] border-white/5 flex flex-col justify-between relative overflow-hidden">
                ...
                <div>
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Mánaðarhraði</p>
                  ...
                  <p className={`text-[9px] font-bold uppercase mt-2 tracking-widest italic ${velocity.pacePercent >= 100 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    {velocity.pacePercent >= 100 ? 'Á áætlun eða yfir' : 'Undir áætlun'}
                  </p>
                </div>
              ...
              <div className="glass p-10 rounded-[48px] border-[#d4af37]/20 flex flex-col justify-between bg-gradient-to-br from-[#d4af37]/5 to-transparent relative">
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="text-xl font-black text-white uppercase italic tracking-tighter">AI Greining</h3>
                  </div>
                  {isLoadingAI ? (
                    ...
                    <p className="text-sm font-bold text-slate-200 leading-relaxed italic">
                      "{aiPulse?.strategicAdvice || "Greini gögn til að veita innsýn..."}"
                    </p>
                  )}
                </div>
                <div className="mt-8 pt-8 border-t border-white/5 flex items-center justify-between">
                  <span className="text-[9px] font-black text-slate-500 uppercase">Öflugasta verkefnið</span>
                  <span className="text-xs font-black text-white uppercase italic tracking-tighter">{aiPulse?.topProject || "Bíð gagna..."}</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="glass p-6 rounded-[32px] border-white/5">
                <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Heildarsala liðsins</p>
              ...
              <div className="glass p-6 rounded-[32px] border-white/5">
                <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Heildarkostnaður (Vinnulaun)</p>
                <p className="text-xl font-black text-rose-400 italic">{formatISK(totalTeamHours * 2724.88)}</p>
              </div>
              <div className="glass p-6 rounded-[32px] border-white/5">
                <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Skilvirkni (ISK/klst)</p>
          ...
          <motion.div key="projects" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} className="space-y-8">
                  ...
                  <div key={name} className="glass p-12 rounded-[56px] border-[#d4af37]/30 bg-gradient-to-br from-[#d4af37]/10 to-transparent relative overflow-hidden shadow-2xl">
                    <div className="flex justify-between items-start mb-12 relative z-10">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                           ...
                           <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest">{name} - Árangur</h4>
                        </div>
                        ...
                        <div className="flex items-center gap-3 mt-4">
                           <div className="flex items-center gap-1.5 px-3 py-1 bg-white/5 rounded-full">
                              ...
                              <p className="text-[10px] font-black text-white uppercase tracking-widest">{formatISK(efficiency)}/klst (Virk)</p>
                           </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-8 relative z-10">
                      <div className="p-8 bg-white/5 rounded-[40px] border border-white/5">
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-3">Söfnunarárangur</p>
                        <div className="space-y-3 text-xs font-black">
                           <div className="flex justify-between items-center"><span className="text-slate-400 uppercase">Söfnun:</span> <span className="text-emerald-400">{formatISK(stats.sales)}</span></div>
                           <div className="flex justify-between items-center"><span className="text-slate-400 uppercase">Kostnaður:</span> <span className="text-rose-400">{formatISK(stats.cost)}</span></div>
                           <div className="pt-2 border-t border-white/5 flex justify-between items-center"><span className="text-slate-400 uppercase">Hagnaður:</span> <span className="text-white">{formatISK(stats.sales - stats.cost)}</span></div>
                        </div>
                      </div>
                      <div className="p-8 bg-white/5 rounded-[40px] border border-white/5">
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-3">Vinnuframlag</p>
                        <div className="space-y-3 text-xs font-black">
                           <div className="flex justify-between items-center"><span className="text-slate-400 uppercase">Stundir:</span> <span className="text-white">{stats.hours}h</span></div>
                           <div className="flex justify-between items-center"><span className="text-slate-400 uppercase">Virkar:</span> <span className="text-indigo-400">{stats.effHours.toFixed(1)}h</span></div>
                           <div className="flex justify-between items-center"><span className="text-slate-400 uppercase">Fjöldi sala:</span> <span className="text-white">{stats.count}</span></div>
                        </div>
                      </div>
                    </div>
            ...
            <div className="glass p-10 rounded-[56px] border-white/5">
              <h3 className="text-xl font-black text-white uppercase italic tracking-tighter mb-10 flex items-center gap-3">
                 <PieIcon className="text-indigo-400" size={24} /> Samanburður á Söfnun
              </h3>
                    ...
                    >
                      <div className="flex items-center gap-6">
                        ...
                        <div className="text-left">
                          ...
                          <p className="text-[9px] font-bold text-slate-600 uppercase tracking-widest">Starfsmanna ID: {agent.staffId} • {agent.team}</p>
                        </div>
                      </div>
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden px-8">
                          <div className="p-8 bg-white/2 border-x border-b border-white/5 rounded-b-[40px] grid grid-cols-1 md:grid-cols-3 gap-6 shadow-inner">
                            <div className="p-5 bg-white/2 rounded-3xl border border-white/5">
                              ...
                              <p className="text-xl font-black text-[#d4af37] italic">{formatISK(agent.efficiency)}<span className="text-[9px] opacity-40 ml-1">/klst</span></p>
                            </div>
                            <div className="p-5 bg-white/2 rounded-3xl border border-white/5">
                              <p className="text-[9px] font-black text-slate-500 uppercase mb-2">Helsta verkefni</p>
                              <p className="text-xl font-black text-white italic uppercase tracking-tighter">{agent.topProject}</p>
                            </div>
                            <div className="p-5 bg-white/2 rounded-3xl border border-white/5">
                              <p className="text-[9px] font-black text-slate-500 uppercase mb-2">Vinnustundir (Brúttó)</p>
                              <p className="text-xl font-black text-white italic">{agent.totalHours}h</p>
                            </div>
                          </div>
                        </motion.div>
                      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        <div className="lg:col-span-8 glass p-10 rounded-[56px] border-indigo-500/20 bg-gradient-to-r from-indigo-500/10 to-transparent flex flex-col md:flex-row items-center gap-10 shadow-2xl">
          <div className="flex items-center gap-6 shrink-0">
            ...
            <div>
               ...
               <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-2">Færðu inn þína sölu hér</p>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 glass p-10 rounded-[56px] border-[#d4af37]/20 flex flex-col justify-between shadow-2xl">
           ...
           <div>
              <label className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-2 block">Heildarsala liðsins (ISK)</label>
